...
void main() {
  ...
  test('mock adapter carries backend reading quiz submission data', () async {
    const adapter = MockCompetitiveEvidenceProviderAdapter();
    ...
    final evidence = await adapter.buildEvidence(
      ...
      quizId: 'quiz-1',
      quizAnswers: const ['ideia principal', 'acao pratica'],
    );

    expect(evidence.quizId, 'quiz-1');
    expect(evidence.answers, const ['ideia principal', 'acao pratica']);
  });
